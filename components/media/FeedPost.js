...
export default function FeedPost({
  ...
  const handleCopyLink = () => {
    const url =
      typeof window !== 'undefined' ? `${window.location.origin}/media/feed/${post._id}` : '';
    ...
    >
      ...
      <div className="flex items-center justify-between p-3">
        ...
        >
          ...
          <div>
            <p className="font-medium text-foreground text-sm">{post.user?.name || 'Unknown'}</p>
            {post.location?.name && (
              <p className="text-xs text-muted-foreground">{post.location.name}</p>
            )}
          </div>
        </Link>
      ...
      <div className="relative bg-black flex justify-center">
        {post.mediaType === 'video' ? (
          <div
            className={`w-full ${post.aspectRatio === '9:16' || !post.aspectRatio ? 'max-w-[400px]' : ''}`}
          >
            <VideoPlayer
      ...
      <div className="p-3">
        <div className="flex items-center gap-4 mb-2">
          ...
          >
            <Heart className={`h-6 w-6 ${loved ? 'fill-current' : ''}`} />
            <span className="text-sm font-medium">{formatCount(loveCount)}</span>
          </button>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 transition-colors ${
              respected ? 'text-yellow-500' : 'text-foreground hover:text-yellow-500'
            }`}
          >
            <span className={`text-xl ${respected ? '' : 'grayscale opacity-70'}`}>🙏</span>
            <span className="text-sm font-medium">{formatCount(respectCount)}</span>
          </button>
        ...
        {post.caption && (
          <p className="text-sm text-foreground mb-2">
            <Link href={`/profile/${post.user?._id}`} className="font-medium hover:underline mr-2">
              {post.user?.name}
            </Link>
            {post.caption}
          </p>
