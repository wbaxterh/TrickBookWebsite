...
import { connectFeedSocket } from '../../lib/socket';
import UserAvatar from '../UserAvatar';
import { Button } from '../ui/button';
import { Input } from '../ui/input';

export default function CommentSection({ postId, initialCommentCount = 0, onCommentCountChange }) {
  ...
  useEffect(() => {
    ...
    socket.on('comment:loved', ({ commentId, loveCount }) => {
      setComments((prev) => prev.map((c) => (c._id === commentId ? { ...c, loveCount } : c)));
    });
    ...
    <div className={`flex gap-3 ${isReply ? 'ml-10 mt-3' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-4 mt-1 text-xs text-muted-foreground">
          ...
          >
            <Heart className={`h-3 w-3 ${comment.loved ? 'fill-current' : ''}`} />
