    ...
    >
      ...
      >
        ...
        {isHovered && video.thumbnails?.preview && (
          <Image src={video.thumbnails.preview} alt="" fill className="object-cover" unoptimized />
        )}
      </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {showRating && video.avgRating > 0 && (
            <div className="flex items-center gap-1">
              ...
              <span className="text-white text-xs">{video.avgRating.toFixed(1)}</span>
            </div>
          )}

          {/* Year */}
          {video.releaseYear && <span className="text-white/70 text-xs">{video.releaseYear}</span>}

          {/* Sport Types */}
          {video.sportTypes?.length > 0 && (
            <span className="text-white/70 text-xs capitalize">{video.sportTypes[0]}</span>
          )}
        </div>
      </div>
