...
export default function Home({ freezeHero = false }) {
  const { t } = useTranslation('home');
  const { loggedIn } = useContext(AuthContext);
  const stats = useStats();
  const existingHeroVariant = useFeatureFlag('hero-headline', 'control');
  const heroVariant = freezeHero ? 'control' : existingHeroVariant;
