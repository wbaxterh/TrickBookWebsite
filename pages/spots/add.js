import { AlertCircle, ArrowLeft, Check, Globe, Info, Link2, Loader2, MapPin } from 'lucide-react';
    ...
    try {
      ...
      if (data.address) {
        ...
        const foundState = Object.entries(STATE_NAMES).find(
          ([_abbr, name]) => name.toLowerCase() === state.toLowerCase(),
        );
        if (foundState) {
    ...
    try {
      ...
      setTimeout(() => {
        const spotSlug = result.name
          ...
          .replace(/(^-|-$)/g, '');
        router.push(
          `/spots/${result.state?.toLowerCase() || 'unknown'}/${spotSlug}?id=${result._id}`,
        );
      }, 1500);
    } catch (err) {
                ...
                <div className="flex items-start gap-3 p-4 bg-muted rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      If checked, this spot will be reviewed and may appear in the public spots
                      directory. Otherwise, it will be saved to your private collection.
                    </p>
                  </div>
                </div>
