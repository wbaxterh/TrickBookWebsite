...
export default function BlogPost({ postData }) {
	...
	const replaceShortcodes = (content, images) => {
		return content.replace(/\[image(\d+)\]/g, (match, p1) => {
			...
			if (images && images[index]) {
				return `<img src="${images[index]}" alt="Image ${
					index + 1
				}" class="blog-image" />`;
			}
			return match;
	...
	const heroImage =
		images && images.find((image) => image.includes("?hero=true"));
	const otherImages =
		images && images.filter((image) => !image.includes("?hero=true"));
	...
	return (
		<>
			<Head>
				...
				<link
					rel='canonical'
					href={`https://thetrickbook.com/blog/${postData.slug}`}
				/>
