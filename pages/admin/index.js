        ...
        <tr className={styles.tableRow} key={tricklist._id}>
            ...
            <table className={styles.table}>
                ...
                <tr className={styles.tableRow}>
                    ...
                    <td className={styles.tableCell} key={trick._id}>{trick.name}, <br/> </td>
                    ))}
                </tr>
                </tbody>
            </table>
...
const toggleTrickLists = () =>{
    ...
    <Head>
      ...
      <Layout>
        ...
        <h1 className="pt-3" style={{textAlign:'center'}}>Current Data</h1>
        <button className="btn btn-light container-fluid my-1" onClick={toggleTable}> {!showTable ? <i className="material-icons" style={{ fontSize: '12px' }}>add</i> : <i className="material-icons" style={{ fontSize: '12px' }}>remove</i>} Users</button>
        { showTable && (
        <UsersTable users={users} />
        )
      }
      <button className="btn btn-light container-fluid my-1" onClick={toggleTrickLists}>{!showTrickLists ? <i className="material-icons" style={{ fontSize: '12px' }}>add</i> : <i className="material-icons" style={{ fontSize: '12px' }}>remove</i>}Trick Lists</button>
        { showTrickLists && (
