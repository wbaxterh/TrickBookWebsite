...
import { getAdminVideos, updateVideo, deleteVideo, syncFromDrive } from "../../../lib/apiMedia";
...
export default function AdminCouch() {
	...
	const handleToggleFeatured = async (video) => {
		try {
			...
			setVideos((prev) =>
				prev.map((v) =>
					v._id === video._id ? { ...v, isFeatured: !v.isFeatured } : v
				)
			);
		} catch (error) {
			console.error("Error updating video:", error);
					...
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
						<div>
							...
							<p className="text-muted-foreground">
								Manage videos, collections, and content
							</p>
						</div>
							...
							<table className="w-full">
								<thead className="bg-secondary">
									<tr>
										...
										<th className="text-left p-4 text-sm font-medium text-muted-foreground hidden md:table-cell">Sport</th>
										<th className="text-left p-4 text-sm font-medium text-muted-foreground hidden lg:table-cell">Year</th>
										<th className="text-left p-4 text-sm font-medium text-muted-foreground hidden lg:table-cell">Size</th>
								...
								<tbody>
									{filteredVideos.map((video) => (
										<tr key={video._id} className="border-t border-border hover:bg-secondary/50">
											<td className="p-4">
												<Link href={`/admin/couch/edit/${video._id}`} className="flex items-center gap-3 group">
													...
													<div className="min-w-0">
														...
														<p className="text-xs text-muted-foreground">
															{video.hlsUrl ? "Bunny.net" : video.driveFileId ? "Google Drive" : "No source"}
														</p>
													</div>
												</Link>
											</td>
											...
											<td className="p-4">
												<div className="flex items-center justify-end gap-1">
													...
													<Link href={`/admin/couch/edit/${video._id}`}>
														<button className="p-2 hover:bg-yellow-500/20 rounded-lg transition-colors" title="Edit metadata">
