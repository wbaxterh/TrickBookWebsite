...
const convertCustomLinks = (content) => {
  const customLinkRegex = /@@(http[s]?[^\s@@]+)@@/g;
  return content.replace(customLinkRegex, '<a target="_blank" href="$1">$1</a>');
};
...
export async function getPostData(slug) {
  ...
  // Replace custom links after Remark has processed the Markdown
  contentHtml = convertCustomLinks(contentHtml);
