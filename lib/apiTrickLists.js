...
// Link/unlink a spot to a trick
export async function linkSpotToTrick(trickId, spotId, token) {
  const response = await axios.put(
    `${TRICK_API_URL}/${trickId}/spot`,
    { spotId },
...
// Link a video URL or feed post to a trick
export async function linkVideoToTrick(trickId, videoUrl, feedPostId, token) {
  const response = await axios.put(
    `${TRICK_API_URL}/${trickId}/video`,
    { videoUrl, feedPostId },
    { headers: { 'x-auth-token': token } },
  );
  return response.data;
