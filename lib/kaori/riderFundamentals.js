 ...
 * made of. Trick timelines (trickAnimations.js) compose these:
 *
 ...
 * TrickList/src/components/companion/riderFundamentals.ts (TS types
 * stripped; every tuning value and axis-convention comment kept intact).
 * The pose math MUST stay identical to mobile — tune there, copy here.
 */
...
export const phase = (t, start, end) => clamp01((t - start) / (end - start));
...
export function hipDropFor(crouch) {
...
export function jumpArc(u, height) {
 ...
 *
 * Channels (see the mobile RiderPose interface for the authoritative docs):
 *  - spin: Yaw rotation progress 0→1 (multiplied by the trick's total spin).
 *  - pitch: Flip progress 0→1 (multiplied by the trick's total FLIP — a
 *    whole-body PITCH about the board's long axis, parallel to `spin`).
 *    0 for pure spins.
 *  - height: Root height above the board line (jump arc).
 *  - crouch: 0 straight legs → 1 full squat (hip drop is derived from this).
 *  - coil: Upper-body coil around Y: negative = wound away from the spin.
 *  - tuck: 0 rest → 1 knees-to-chest airborne tuck.
 *  - balance: 0 → 1 arms out wide for landing balance.
 *  - headLead / headSpot: Head: lead the spin (yaw) and spot the landing
 *    (pitch down).
 *  - headRoll: Head tilt/roll toward the leading shoulder (radians) — "head
 *    over the shoulder".
 *  - backLegLift / frontLegLift: Asymmetric leg raise for stylish variants
 *    (0 → 1). Regular stance: front = left leg (lead foot), back = right leg.
 *    Lifting a leg bends its knee up so the corresponding end of the board
 *    can angle up.
 *  - boardTilt: Board angle around its long axis (radians; + = tail up /
 *    nose down).
 *  - grabFront/grabRear/grabReach/grabSide: GRAB channels (all 0 = no grab;
 *    existing tricks unaffected). grabFront/grabRear: 0→1 reach of the FRONT
 *    (lead/left) or REAR (right) hand down to the board. grabReach: where
 *    along the board the hand aims, -1 = nose (+X) … 0 = between the feet …
 *    +1 = tail. grabSide: +1 = toe edge (her facing side), -1 = heel edge
 *    (reach behind).
 *  - pokeFront / pokeBack: Leg POKE (bone): 0→1 straightens that knee and
 *    pushes that end of the board away — the opposite of a lift. Nosebone =
 *    front poke.
 *  - arch: Back arch (method/japan tweak): 0→1 bends the spine back, chest
 *    opens.
 *  - edgeLean: Carve lean: -1 heel-side … +1 toe-side whole-torso lean
 *    (radians ≈ ×0.5).
 *  - dir: Spin direction: +1 frontside (CCW / +Y), -1 backside (CW / -Y).
 *    Flips the body wind, head look and arm wrap so a backside READS as
 *    backside. The actual rotation is carried by the trick's signed
 *    totalSpin, and `coil` stays frontside-signed either way so the arm
 *    load→whip TIMING is preserved (applyArms keys windup/whip off the coil
 *    sign).
 */
...
export function stanceIdlePose(idleT) {
  return {
    ...REST_POSE,
...
function applyTorsoAndHead(humanoid, pose, w) {
...
function applyGrabReach(upper, lower, sign, pose, grabAmt, otherGrab) {
...
function applyArms(humanoid, pose, w) {
...
export function applyRiderPose(humanoid, pose, stanceWeight) {
  applyLegs(
    humanoid,
...
export function resetRiderBones(humanoid) {
...
export function windUpDemoPose(t) {
...
export function landDemoPose(t) {
