 ...
 * fundamentals (riderFundamentals.js): stance, crouch + hip drop, coil
 ...
 * (+Y yaw in three.js).
 *
 * Ported 1:1 from the mobile app's
 * TrickList/src/components/companion/trickAnimations.ts (TS types
 * stripped; every timeline, tuning value and axis-convention comment kept
 * intact). The pose math MUST stay identical to mobile — tune there, copy
 * here.
 *
 * TrickTimeline shape (per-entry in TRICKS):
 *  - duration: seconds.
 *  - totalSpin: Total root YAW over the trick (radians; + is
 *    frontside/CCW).
 *  - totalFlip: Total root PITCH/flip over the trick (radians, SIGNED:
 *    positive = wildcat, end over end in the TAIL direction — nose up and
 *    over backward; negative = tamedog, over the nose). Omit for pure
 *    spins (treated as 0). Yaw is about vertical, pitch about the local
 *    toe-heel axis (end over end) — the two compose independently. For
 *    corks/rodeos, totalFlip is the PEAK off-axis angle and pose.pitch is
 *    a bell that returns to 0 by landing.
 *  - yawOffset: Constant stance-yaw offset (radians), eased by stance
 *    weight. π = the trick STARTS switch (cab tricks). Omit for
 *    regular-stance tricks.
 *  - poseAt(t): RiderPose at t seconds.
 */
...
/** Frontside 360 — CCW / +Y. */
function frontside360PoseAt(t) {
  return spin360PoseAt(t, 1);
}

/** Backside 360 — CW / -Y (mirror). */
function backside360PoseAt(t) {
  return spin360PoseAt(t, -1);
}
 ...
 */
function frontside360StylishPoseAt(t) {
 ...
 */
function backside360StylishPoseAt(t) {
...
function makeSpin(revs, dir, yawOffset = 0) {
  const SETUP = 1.5;
  const POP = SETUP + 0.3;
...
function makeCork(revs, dir, corkPeak) {
...
// GrabSpec fields: front/rear (0..1 hand grab amount), reach (-1 nose … 0
// between feet … +1 tail), side (+1 toe edge, -1 heel edge), tuck
// (knees-to-board fold, default 0.5 — brings the board in reach), pokeFront,
// pokeBack, frontLegLift, backLegLift, arch, balance (arms-out styling for
// grab-less straight air).
function makeGrabAir(spec) {
  const SETUP = 1.1;
...
// --- Demo state machine ---
//
// TrickDemoState fields (see the mobile interface for the authoritative docs):
//  - session: On-board demo session — enter when the demo reply starts.
//  - trick / action / actionT
//  - gapT: Time spent in stance since the last action — drives the auto-loop.
//  - holdT: Post-activity grace timer. Board presence is NOT tied to the exact
...
//  - stance: Smoothed on-board weight (drives board fade + stance width + yaw).
//  - idleT
//  - rootYaw / rootY: Outputs for the frame loop + board renderer.
//  - rootPitch: Whole-body flip angle this frame (radians, signed). 0 for spins.
//  - rootQuat: The composed whole-body quaternion [x,y,z,w] KaoriStage applied
//    this frame (yaw*pitch). Written by KaoriStage; read by lockBoardToFeet so
//    the board rolls with her through a flip inversion instead of staying
//    world-flat.
//  - boardY: Board height — follows the jump but NOT the crouch hip-drop.
//  - boardOpacity
//  - boardTilt: Board angle around its long axis (tail up/down) for stylish
//    variants.
//  - boardPos / boardQuat: Board world transform locked to the actual feet
//    (computed in KaoriStage from the raw foot bones so the bindings stay under
//    the soles and the board angle follows the feet). Plain arrays to keep this
//    module three-free.
//  - boardLocked

export const createTrickDemoState = () => ({
  session: false,
...
export const isDemoActive = (state) =>
...
export function startAction(state, action) {
 ...
 */
export function driveDemo(vrm, state, dt) {
