...
const eventQueue = [];
let flushTimer = null;
const HOMEPAGE_CONTEXT_KEY = 'tb_homepage_experiment';

export function setHomepageExperiment(homepage) {
  if (typeof window === 'undefined') return;
  try {
    if (homepage.mode === 'experiment') {
      sessionStorage.setItem(
        HOMEPAGE_CONTEXT_KEY,
        JSON.stringify({ experiment: homepage.name, homepage_variant: homepage.variant }),
      );
    } else {
      sessionStorage.removeItem(HOMEPAGE_CONTEXT_KEY);
    }
  } catch {
...
function homepageContext() {
  if (typeof window === 'undefined') return {};
  try {
    return JSON.parse(sessionStorage.getItem(HOMEPAGE_CONTEXT_KEY) || '{}');
  } catch {
    return {};
  }
}
...
function track(event, properties) {
  if (process.env.NEXT_PUBLIC_HOMEPAGE_PREVIEW === 'true') return;
  const attributedProperties = { ...homepageContext(), ...properties };
  // PostHog
  try {
    if (posthog.__loaded) posthog.capture(event, attributedProperties);
  } catch {
    /* Analytics must not interrupt rider actions. */
  }
  try {
    sendToBackend(event, attributedProperties);
  } catch {
    /* Storage may be unavailable in private browsing. */
  }
}

export function trackHomepageExposure(variant) {
  track('homepage_experiment_viewed', {
    experiment: 'homepage-design-v1',
    homepage_variant: variant,
  });
}

export function trackHomepageDemo(action, sport) {
  track('homepage_demo_interacted', { action, sport });
}
