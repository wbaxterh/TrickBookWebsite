...
export function getEventAction(event) {
  if (event?.status === 'completed' || (event?.endAt && new Date(event.endAt) < new Date())) {
    return { label: 'View event', url: null, kind: 'details' };
  }
  ...
  const officialUrl = event?.externalLinks?.find((link) => link.kind === 'official')?.url;
  return {
    label: 'View official details',
    url: officialUrl || event?.sourceUrl || null,
    kind: 'details',
  };
}
...
export function getEventStatus(event) {
  if (event?.status === 'completed' || (event?.endAt && new Date(event.endAt) < new Date())) {
    return { label: 'Past event', tone: 'past' };
  }
