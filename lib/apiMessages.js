import axios from 'axios';

const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'https://api.thetrickbook.com/api';

/**
 * Get all conversations for the current user
 * @param {string} token - JWT auth token
 * @returns {Promise<Array>} List of conversations with other user info
 */
export async function getConversations(token) {
  const response = await axios.get(`${API_BASE_URL}/dm/conversations`, {
    headers: { 'x-auth-token': token },
  });
  return response.data;
}

/**
 * Get a single conversation by ID
 * @param {string} conversationId - The conversation ID
 * @param {string} token - JWT auth token
 * @returns {Promise<Object>} Conversation with other user info
 */
export async function getConversation(conversationId, token) {
  const response = await axios.get(`${API_BASE_URL}/dm/conversations/${conversationId}`, {
    headers: { 'x-auth-token': token },
  });
  return response.data;
}

/**
 * Get messages for a conversation
 * @param {string} conversationId - The conversation ID
 * @param {Object} options - Pagination options
 * @param {number} options.page - Page number (default 1)
 * @param {number} options.limit - Messages per page (default 50)
 * @param {string} token - JWT auth token
 * @returns {Promise<Object>} Messages array with pagination
 */
export async function getMessages(conversationId, { page = 1, limit = 50 } = {}, token) {
  const response = await axios.get(
    `${API_BASE_URL}/dm/conversations/${conversationId}/messages?page=${page}&limit=${limit}`,
    {
      headers: { 'x-auth-token': token },
    },
  );
  return response.data;
}

/**
 * Start a new conversation with a user (or get existing one)
 * @param {string} targetUserId - The user ID to start conversation with
 * @param {string} token - JWT auth token
 * @returns {Promise<Object>} Conversation object
 */
export async function startConversation(targetUserId, token) {
  const response = await axios.post(
    `${API_BASE_URL}/dm/conversations`,
    { targetUserId },
    {
      headers: { 'x-auth-token': token },
    },
  );
  return response.data;
}

/**
 * Send a message in a conversation
 * @param {string} conversationId - The conversation ID
 * @param {string} content - Message content
 * @param {string} token - JWT auth token
 * @returns {Promise<Object>} Created message object
 */
export async function sendMessage(conversationId, content, token, extraHeaders) {
  const response = await axios.post(
    `${API_BASE_URL}/dm/conversations/${conversationId}/messages`,
    { content },
    {
      headers: { 'x-auth-token': token, ...extraHeaders },
    },
  );
  return response.data;
}

/**
 * Send a shared-content message (spot, trick list, video, etc.) in a conversation.
 * Mirrors the mobile app's share flow. Backend accepts { content?, sharedContent }.
 * @param {string} conversationId - The conversation ID
 * @param {Object} sharedContent - { contentType, contentId, preview? }
 * @param {string} [content] - Optional accompanying text
 * @param {string} token - JWT auth token
 * @returns {Promise<Object>} Created message object
 */
export async function sendSharedContent(conversationId, sharedContent, content, token) {
  const response = await axios.post(
    `${API_BASE_URL}/dm/conversations/${conversationId}/messages`,
    { content: content || undefined, sharedContent },
    {
      headers: { 'x-auth-token': token },
    },
  );
  return response.data;
}

/**
 * Mark a conversation as read
 * @param {string} conversationId - The conversation ID
 * @param {string} token - JWT auth token
 * @returns {Promise<Object>} Success response
 */
export async function markAsRead(conversationId, token) {
  const response = await axios.put(
    `${API_BASE_URL}/dm/conversations/${conversationId}/read`,
    {},
    {
      headers: { 'x-auth-token': token },
    },
  );
  return response.data;
}

/**
 * Get total unread message count across all conversations
 * @param {string} token - JWT auth token
 * @returns {Promise<Object>} Object with unreadCount property
 */
export async function getUnreadCount(token) {
  const response = await axios.get(`${API_BASE_URL}/dm/unread-count`, {
    headers: { 'x-auth-token': token },
  });
  return response.data;
}

/**
 * Get list of bot companions available for chat
 * @param {string} token - JWT auth token
 * @returns {Promise<Array>} List of bot users with existingConversationId
 */
export async function getBotCompanions(token) {
  const response = await axios.get(`${API_BASE_URL}/dm/bots`, {
    headers: { 'x-auth-token': token },
  });
  return response.data;
}

/**
 * Start or get conversation with a bot (no homie requirement)
 * @param {string} botId - The bot user ID
 * @param {string} token - JWT auth token
 * @returns {Promise<Object>} Conversation object
 */
export async function startBotConversation(botId, token) {
  const response = await axios.post(
    `${API_BASE_URL}/dm/bot-conversation`,
    { botId },
    {
      headers: { 'x-auth-token': token },
    },
  );
  return response.data;
}

/**
 * Get the companion relationship profile for a bot
 * @param {string} companionId - The bot user ID
 * @param {string} token - JWT auth token
 * @returns {Promise<Object>} Relationship profile
 */
export async function getCompanionProfile(companionId, token) {
  const response = await axios.get(`${API_BASE_URL}/companion/profile/${companionId}`, {
    headers: { 'x-auth-token': token },
  });
  return response.data;
}

/**
 * Request a context-aware greeting from the companion
 * @param {string} companionId - The bot user ID
 * @param {string} token - JWT auth token
 * @returns {Promise<Object>} { greeting, stage }
 */
export async function getCompanionGreeting(companionId, token) {
  const response = await axios.post(
    `${API_BASE_URL}/companion/profile/${companionId}/greeting`,
    {},
    { headers: { 'x-auth-token': token } },
  );
  return response.data;
}
