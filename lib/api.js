...
const LOCAL_POSTS_DIRECTORY = path.join(process.cwd(), 'posts');

function getLocalPostFileNames() {
  if (!fs.existsSync(LOCAL_POSTS_DIRECTORY)) {
    return [];
  }

  return fs.readdirSync(LOCAL_POSTS_DIRECTORY).filter((fileName) => fileName.endsWith('.md'));
}

function resolveLocalImagePath(slug, imagePath) {
  if (!imagePath || imagePath.startsWith('http') || imagePath.startsWith('/')) {
    return imagePath;
  }

  return `/images/${slug}/${imagePath.replace(/^\.\//, '')}`;
}

function normalizeMarkdownImages(content, slug) {
  return content.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (_match, altText, imagePath) => {
    return `![${altText}](${resolveLocalImagePath(slug, imagePath)})`;
  });
}

function getLocalPostSummaries() {
  return getLocalPostFileNames()
    .map((fileName) => {
      const slug = fileName.replace(/\.md$/, '');
      const fullPath = path.join(LOCAL_POSTS_DIRECTORY, fileName);
      const fileContents = fs.readFileSync(fullPath, 'utf8');
      const matterResult = matter(fileContents);
      const normalizedContent = normalizeMarkdownImages(matterResult.content, slug);
      const images = Array.from(
        normalizedContent.matchAll(/!\[[^\]]*\]\(([^)]+)\)/g),
        (match) => match[1],
      );
      ...
      return {
        ...
        author: matterResult.data.author || 'TrickBook',
...
async function getLocalPostData(slug) {
  const fullPath = path.join(LOCAL_POSTS_DIRECTORY, `${slug}.md`);
  ...
  const matterResult = matter(fileContents);
  const normalizedContent = normalizeMarkdownImages(matterResult.content, slug);
  const processedContent = await remark().use(html).process(normalizedContent);
  const content = processedContent.toString();
  const images = Array.from(content.matchAll(/<img[^>]*src="([^"]+)"/g), (match) => match[1]);
  ...
  return {
    ...
    deck: matterResult.data.deck || null,
    heroImageAlt: matterResult.data.heroImageAlt || null,
    content,
    images,
  };
}
...
export async function getAllPostIds() {
  try {
    ...
    return getLocalPostSummaries().map((post) => ({
      params: {
        slug: post.url,
      },
    }));
...
export async function getAdjacentPosts(slug) {
  try {
    ...
    const currentIndex = allPosts.findIndex((p) => p.url === slug);
