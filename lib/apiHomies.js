...
// Supports ?q= search and ?page=&limit= pagination
export async function getDiscoverableUsers(token, { q, page, limit } = {}) {
  const params = new URLSearchParams();
  if (q) params.append('q', q);
  if (page) params.append('page', page);
  if (limit) params.append('limit', limit);
  const qs = params.toString();
  const response = await axios.get(`${API_BASE_URL}/discoverable${qs ? `?${qs}` : ''}`, {
    headers: {
      'x-auth-token': token,
    },
  });
