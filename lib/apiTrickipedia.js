import axios from 'axios';

const BASE_URL = process.env.NEXT_PUBLIC_BASE_URL || 'https://api.thetrickbook.com';
const TRICKS_API_URL = `${BASE_URL}/api/trickipedia`;
const TRICK_IMAGE_API_URL = `${BASE_URL}/api/trickImage`;
const CATEGORIES_API_URL = `${BASE_URL}/api/categories`;

// Get all tricks (with optional category filter)
export async function getSortedTricksData(category, token) {
  try {
    // The API defaults to 100 results, which silently hid alphabetically later
    // tricks (including both Ollie records) as the catalog grew beyond that.
    const response = await axios.get(TRICKS_API_URL, {
      params: { limit: 250 },
      headers: token ? { 'x-auth-token': token } : {},
    });
    let allTricksData = response.data.map((trick) => {
      const {
        _id,
        name,
        category,
        difficulty,
        description,
        steps,
        images,
        videoUrl,
        videos,
        source,
        url,
        sampleData,
        tips,
        commonMistakes,
        safety,
        tutorials,
        progression,
      } = trick;
      return {
        id: _id,
        name,
        category,
        difficulty,
        description,
        steps,
        images,
        videoUrl,
        videos,
        source,
        url,
        sampleData,
        tips,
        commonMistakes,
        safety,
        tutorials,
        progression,
      };
    });
    if (category) {
      allTricksData = allTricksData.filter((trick) => trick.category === category);
    }
    // Preserve the API's personalized ranking. Anonymous responses are already
    // alphabetical, so callers retain the existing fallback behavior.
    return allTricksData;
  } catch (_error) {
    return [];
  }
}

export async function getTrickNetwork(id) {
  try {
    const response = await axios.get(`${TRICKS_API_URL}/${id}/network`);
    return response.data;
  } catch (_error) {
    return null;
  }
}

// Get all tricks but surface errors (for admin views that need to
// distinguish a failed/expired-token load from a truly empty result).
export async function getSortedTricksDataOrThrow(category) {
  const response = await axios.get(TRICKS_API_URL, { params: { limit: 250 } });
  let allTricksData = response.data.map((trick) => {
    const {
      _id,
      name,
      category: trickCategory,
      difficulty,
      description,
      steps,
      images,
      videoUrl,
      videos,
      source,
      url,
      sampleData,
    } = trick;
    return {
      id: _id,
      name,
      category: trickCategory,
      difficulty,
      description,
      steps,
      images,
      videoUrl,
      videos,
      source,
      url,
      sampleData,
    };
  });
  if (category) {
    allTricksData = allTricksData.filter((trick) => trick.category === category);
  }
  return allTricksData.sort((a, b) => a.name.localeCompare(b.name));
}

// Get a single trick by ID
export async function getTrickData(id) {
  try {
    const response = await axios.get(`${TRICKS_API_URL}/${id}`);
    return response.data;
  } catch (_error) {
    return null;
  }
}

// Create a new trick
export async function createTrick(trickData, token) {
  const response = await axios.post(TRICKS_API_URL, trickData, {
    headers: {
      'x-auth-token': token,
    },
  });
  return response.data;
}

// Update an existing trick
export async function updateTrick(id, trickData, token) {
  const response = await axios.put(`${TRICKS_API_URL}/${id}`, trickData, {
    headers: {
      'x-auth-token': token,
    },
  });
  return response.data;
}

// Delete a trick
export async function deleteTrick(id, token) {
  const response = await axios.delete(`${TRICKS_API_URL}/${id}`, {
    headers: {
      'x-auth-token': token,
    },
  });
  return response.data;
}

// Upload a trick image
export async function uploadTrickImage(file, trickUrl, token) {
  const formData = new FormData();
  formData.append('file', file);
  const response = await axios.post(
    `${TRICK_IMAGE_API_URL}/upload?trickUrl=${encodeURIComponent(trickUrl)}`,
    formData,
    {
      headers: {
        'x-auth-token': token,
        'Content-Type': 'multipart/form-data',
      },
    },
  );
  return response.data.imageUrl;
}

// Delete all images for a trick
export async function deleteTrickImages(trickUrl, token) {
  const response = await axios.delete(
    `${TRICK_IMAGE_API_URL}/delete-folder/${encodeURIComponent(trickUrl)}`,
    {
      headers: {
        'x-auth-token': token,
      },
    },
  );
  return response.data;
}

// Get all categories
export async function getCategories() {
  try {
    const response = await axios.get(CATEGORIES_API_URL);
    return response.data;
  } catch (_error) {
    return [];
  }
}

// Build a slug-to-name map from categories
export function buildCategoryMap(categories) {
  const map = {};
  for (const cat of categories) {
    const slug = cat.name.toLowerCase().replace(/\s+/g, '-');
    map[slug] = cat.name;
  }
  return map;
}

// Get tricks by category
export async function getTricksByCategory(category) {
  try {
    const response = await axios.get(`${TRICKS_API_URL}/category/${category}`);
    return response.data;
  } catch (_error) {
    return [];
  }
}

// Search tricks
export async function searchTricks(query, category = null, difficulty = null) {
  try {
    let url = `${TRICKS_API_URL}?search=${encodeURIComponent(query)}`;
    if (category) url += `&category=${encodeURIComponent(category)}`;
    if (difficulty) url += `&difficulty=${encodeURIComponent(difficulty)}`;

    const response = await axios.get(url);
    return response.data;
  } catch (_error) {
    return [];
  }
}
